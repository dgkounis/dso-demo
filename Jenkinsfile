pipeline {
  environment {
    ARGO_SERVER = '35.196.62.194:32100'
    DEV_URL = 'http://35.196.62.194:30080/'
  }
  agent {
    kubernetes {
    ...
    stage('Package') {
      parallel {
        ...
        stage('OCI Image BnP') {
          steps {
            container('kaniko') {
              sh '/kaniko/executor -f `pwd`/Dockerfile -c `pwd` --insecure --skip-tls-verify --cache=true --destination=docker.io/dgkounis/dso-demo'
            }
          }
        }
      }
        ...
        stage('Image Linting') {
          steps {
            container('docker-tools') {
              sh 'dockle docker.io/dgkounis/dso-demo'
            }
          }
        }
        stage('Image Scan') {
